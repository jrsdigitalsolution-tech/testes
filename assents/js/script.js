...
function setFilter(status) {
  ...
  function calcularStatusComprasVirtual(r) {
    ...
    for (let j = COLS.ITEM_INICIO; j <= COLS.ITEM_FIM; j++) {
      ...
      const statusOriginal = String(r && r[j] || "").trim();
      const status = statusOriginal.toUpperCase();
      ...
      const statusNA = status === "N/A" || status === "NA" || status === "NAO_APLICA" || status === "NÃO_APLICA";
      const statusPendencia = status === "?" || status === "PENDENTE" || status === "DUVIDA" || status === "DÚVIDA";
      const temDataStatus = isStatusDate(statusOriginal);
      ...
      const temDetalheManual = Boolean(
        String(det.descricao || det.alerta_descricao || det.fornecedor || det.preco || "").trim()
      );

      const itemMonitorado = Boolean(
        status === "OK" ||
        statusParcial ||
        statusPendencia ||
        temDataStatus ||
        temChegadaConfirmada ||
        temOcVinculada ||
        (!statusNA && temDetalheManual)
      );

      if (!itemMonitorado) continue;
    ...
    if (totalMonitorado === 0) {
      return {
        ...
        faltam: 0,
        classe: "days-info",
        titulo: "Nenhum item de compra aplicável/monitorado nesta obra."
      };
    }
    ...
    return {
      ...
      faltam,
      classe,
      titulo: `${totalRecebido} item(ns) recebido(s), ${faltam} pendente(s) de ${totalMonitorado} aplicável(is)/monitorado(s).`
    };
  }
