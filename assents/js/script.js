...
function setFilter(status) {
  ...
  function renderizar(dadosOriginais) {
    ...
    let totalObrasResumo = dados.length;
    if (currentStatusFilter === 'FIRMADAS') {
      const obrasUnicas = new Set();
      dados.forEach(item => {
        const row = Array.isArray(item && item.content) ? item.content : [];
        const detalhes = safeJsonParse(row[COLS.DETALHES_JSON], {});
        const chaveObra = String(detalhes.meta_obra_key || row[COLS.OBRA] || '').trim();
        if (chaveObra) obrasUnicas.add(chaveObra);
      });
      totalObrasResumo = obrasUnicas.size || dados.length;
    }

    const custoMedio = totalObrasResumo > 0 ? (totVal / totalObrasResumo) : 0;
