...
function setFilter(status) {
  ...
  function getObraResumoKey(row) {
    ...
    const key = String(detalhes.meta_obra_key || "").trim();
    if (key) return key;

    return String((row && row[COLS.OBRA]) || "")
      .replace(/\D/g, "")
      .slice(0, 5) || String((row && row[COLS.OBRA]) || "").trim();
  }

  function contarObrasReaisCarteira(dados) {
    const chaves = new Set();
  ...
  function renderizar(dadosOriginais) {
    ...
    const totalObrasResumo = currentStatusFilter === 'FIRMADAS' ? contarObrasReaisCarteira(dados) : dados.length;
