const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function getChaveEntregueObraPedido(obraKey, pedido) {
    const obra = String(obraKey || '').trim();
    const num = String(pedido || '').trim();
    if (!obra || !num) return '';
    return `${obra}::${num}`;
  }

  function getPedidosLinhaEntregue(row) {
    const detalhes = safeJsonParse(row && row[COLS.DETALHES_JSON], {});
    const pedidos = [];

    if (Array.isArray(detalhes.meta_numeros_pedido_linha)) {
      pedidos.push(...detalhes.meta_numeros_pedido_linha);
    }

    pedidos.push(detalhes.meta_numero_pedido_linha);
    pedidos.push(detalhes.meta_numero_pedido);

    if (detalhes.meta_pedido_operacional && detalhes.meta_pedido_operacional.numero_pedido) {
      pedidos.push(detalhes.meta_pedido_operacional.numero_pedido);
    }

    if (Array.isArray(detalhes.meta_pedidos_operacionais_linha)) {
      detalhes.meta_pedidos_operacionais_linha.forEach(pedido => {
        if (pedido && pedido.numero_pedido) pedidos.push(pedido.numero_pedido);
      });
    }

    return deduplicarPedidosDisplay(normalizarListaPedidoDisplay(pedidos));
  }

  function getChavesFiscaisEntregue(dadosFiscais) {
    const chaves = new Set();

    (Array.isArray(dadosFiscais) ? dadosFiscais : []).forEach(item => {
      const row = Array.isArray(item && item.content) ? item.content : [];
      const detalhes = safeJsonParse(row[COLS.DETALHES_JSON], {});
      const obraKey = String(detalhes.meta_obra_key || getObraKeyResumo(row) || '').trim();
      const pedidos = getPedidosLinhaEntregue(row);

      pedidos.forEach(pedido => {
        const chave = getChaveEntregueObraPedido(obraKey, pedido);
  ...
  function expandirLinhasEntregueOperacionalSemFiscal(dados, chavesFiscais) {
    const chaves = chavesFiscais instanceof Set ? chavesFiscais : new Set();
    ...
    return (Array.isArray(dados) ? dados : []).flatMap(item => {
      ...
      const pedidosEntreguesSemFiscal = pedidos.filter(pedido => {
        if (!pedido || !isStatusEntregue(pedido.status_operacional)) return false;

        const obraKey = String((pedido && pedido.meta_obra_key) || detalhes.meta_obra_key || getObraKeyResumo(row) || '').trim();
        const numeroPedido = String((pedido && pedido.numero_pedido) || '').trim();
        const chave = getChaveEntregueObraPedido(obraKey, numeroPedido);

        return !chave || !chaves.has(chave);
      });

      return pedidosEntreguesSemFiscal.map((pedido, idx) => criarLinhaEntregueAPartirPedido(item, row, detalhes, pedido, idx));
    });
  }
  ...
  function getValorResumoLinha(row) {
    const detalhes = safeJsonParse(row && row[COLS.DETALHES_JSON], {});
    if (isFiltroEntregueAtual() && detalhes && detalhes.meta_contexto_linha === "ENTREGUE_FISCAL_MES") {
      const valorContabilizado = parseMoneyFlexible(detalhes.meta_valor_faturamento_contabilizado);
      return Number.isFinite(valorContabilizado) ? valorContabilizado : 0;
    }
  ...
  function solicitarDecisaoRecebimentoCompra(qtdSelecionadas, statusAtual) {
    const qtd = Math.max(parseInt(qtdSelecionadas, 10) || 0, 0);
    const parcialAtual = parseStatusParcial(statusAtual);
    const sugestaoTipo = parcialAtual ? 'P' : 'C';

    const resposta = window.prompt(
      'Recebimento completo ou parcial?\n\nDigite C para completo ou P para parcial.',
      sugestaoTipo
    );

    if (resposta === null) return null;

    const escolha = normalizarEscolhaRecebimento(resposta);

    if (['C', 'COMPLETO', 'TOTAL'].includes(escolha)) {
      return {
        tipo: 'COMPLETO',
        status: 'OK',
        totalPrevisto: qtd,
    ...
    if (!['P', 'PARCIAL'].includes(escolha)) {
      notify("<i class='bi bi-exclamation-triangle me-2'></i> Informe C para completo ou P para parcial.");
      return null;
    }

    const sugestaoTotal = parcialAtual && parcialAtual.totalPrevisto > qtd ? parcialAtual.totalPrevisto : qtd + 1;
    const totalRaw = window.prompt(
      `Total de OCs previstas para este item?\n\nOCs já selecionadas: ${qtd}.\nPara parcial, o total precisa ser maior que ${qtd}.`,
      String(sugestaoTotal)
    );

    if (totalRaw === null) return null;

    const totalPrevisto = parseInt(String(totalRaw || '').replace(/\D/g, ''), 10);

    if (!Number.isFinite(totalPrevisto) || totalPrevisto <= qtd) {
      notify(`<i class='bi bi-exclamation-triangle me-2'></i> Para recebimento parcial, informe um total maior que ${qtd}.`);
      return null;
    }
...
function setFilter(status) {
  ...
  function calcularStatusComprasVirtual(r) {
    ...
    for (let j = COLS.ITEM_INICIO; j <= COLS.ITEM_FIM; j++) {
      ...
      const status = String(r && r[j] || "").trim().toUpperCase();
      const id = getSafeId(itemNome);
      const det = detalhesJson[id] && typeof detalhesJson[id] === 'object' ? detalhesJson[id] : {};

      const temChegadaConfirmada = det.chegada && parseDataUniversal(det.chegada) !== null;
      const temOcVinculada = Boolean(
        String(det.oc || det.pednum || det.numero_pedido_oc || "").trim()
      );
      const statusParcial = parseStatusParcial(status);

      if (statusParcial) {
        const pedidosParciais = deduplicarPedidosDisplay(normalizarListaPedidoDisplay(det.oc || det.pednum || det.numero_pedido_oc || ""));
        if (pedidosParciais.length >= statusParcial.totalPrevisto) {
          totalRecebido += 1;
        }
        continue;
      }

      if (status === "OK" || temChegadaConfirmada || temOcVinculada) {
        totalRecebido += 1;
      }
    }
    ...
    return {
      ...
      faltam,
      classe,
      titulo: `${totalRecebido} item(ns) recebido(s), ${faltam} pendente(s) de ${totalMonitorado} monitorado(s).`
    };
  }
  ...
  function renderizar(dadosOriginais) {
    ...
    if (isFiltroCarteiraAtual()) {
      ...
      if (dadosEntregueFiscal.length) {
        dadosPreparados = dadosEntregueFiscal;
      } else {
        dadosPreparados = expandirLinhasConcluidasPorMes(dadosFiltradosStatus);
      }
    } else {
      const dadosFiltradosStatus = dadosOriginais.filter(d => {
        return statusLinhaCorrespondeFiltro(d.content[COLS.STATUS_PROPOSTA]);
    ...
    if (!isGeralView) {
      ...
      const labs = ["OBRA", "VALOR", "PEDIDO", "CLIENTE", "ITEM", "CATEGORIA", "STATUS DO PRAZO", "STATUS DE COMPRAS", "OBSERVAÇÕES"];
      ...
      dadosOrdenados.forEach((dO, renderIdx) => {
        ...
        html += `<td><span class="days-badge ${resCompras.classe || (resCompras.valor >= 100 ? "days-ok" : "days-urgent")} shadow-sm" title="${escapeHtml(resCompras.titulo || '')}">${resCompras.texto}</span></td>`;

        let miniBadgesMobile = "";
        // Prévia dos itens removida da tabela principal da Carteira.
        // Os detalhes permanecem disponíveis ao abrir a página/modal de edição da obra.
  ...
  function atualizarResumoSelecaoOCCompra() {
    ...
    if (!selecionadas.length) {
      resumo.innerHTML = `
        <span class="oc-picker-summary-muted">Nenhuma OC selecionada.</span>
        <strong>Selecione uma ou mais OCs para liberar o OK.</strong>
      `;
      return;
    }
    ...
    resumo.innerHTML = `
      <span>${selecionadas.length} OC${selecionadas.length > 1 ? 's' : ''} selecionada${selecionadas.length > 1 ? 's' : ''}</span>
      <strong>${escapeHtml(pedidos)} · R$ ${escapeHtml(formatMoneyCompra(valorTotal))}</strong>
    `;
  }
  ...
  async function selecionarDataFaturamento(id) {
    ...
    if (statusAtual && obra && itemLabel && window.motorCompras && typeof window.motorCompras.salvarStatusItemObra === 'function') {
      try {
  ...
  async function salvarPopUpCompra() {
    ...
    if (mode === 'OK') {
      ...
      const decisaoRecebimento = solicitarDecisaoRecebimentoCompra(
      ...
      try {
        ...
        if (window.motorCompras && typeof window.motorCompras.salvarStatusItemObra === 'function') {
          ...
          await window.motorCompras.salvarStatusItemObra(obra, itemLabel, decisaoRecebimento.status, {
  ...
  function editar(idx) {
    ...
    const registroComPersistencia = aplicarStatusItensPersistidos([dadosLocais[idx]])[0] || dadosLocais[idx];
    const r = Array.isArray(registroComPersistencia.content) ? registroComPersistencia.content : dadosLocais[idx].content;
  ...
  function obterUltimaChegadaRow(row) {
    ...
    ITENS.forEach((item, idx) => {
      ...
      const status = String(r && r[COLS.ITEM_INICIO + idx] || '').trim();
      if (isStatusParcial(status)) return;
