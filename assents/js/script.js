const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function getObraKeyResumoCard(row, fallbackIndex) {
    const detalhes = safeJsonParse(row && row[COLS.DETALHES_JSON], {});
    const metaKey = String(detalhes.meta_obra_key || '').trim();
    if (metaKey) return metaKey;

    const obraTxt = String((row && row[COLS.OBRA]) || '').trim();
    const digits = obraTxt.replace(/\D/g, '');
    if (digits.length >= 5) return digits.slice(0, 5);

    return `linha_${fallbackIndex}`;
  }

  function contarObrasResumo(dados) {
    if (currentStatusFilter !== 'FIRMADAS') return dados.length;

    const obras = new Set();
    (Array.isArray(dados) ? dados : []).forEach((item, index) => {
      const row = Array.isArray(item && item.content) ? item.content : [];
      obras.add(getObraKeyResumoCard(row, index));
    });

    return obras.size || dados.length;
  }
...
function setFilter(status) {
  ...
  function renderizar(dadosOriginais) {
    ...
    const qtdObrasResumo = contarObrasResumo(dados);
