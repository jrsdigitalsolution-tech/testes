...
function setFilter(status) {
  ...
  function normalizarObraKeyResumo(value) {
    const raw = String(value ?? "").trim();
    if (!raw) return "";

    const matchSeparador = raw.match(/(\d{2})\s*[.,\-\/]\s*(\d{2,3})/);
    if (matchSeparador) {
      return `${matchSeparador[1]}${String(matchSeparador[2]).padStart(3, '0')}`;
    }

    const matchCincoDigitos = raw.match(/\b(\d{5})\b/);
    if (matchCincoDigitos) return matchCincoDigitos[1];

    return raw.toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^A-Z0-9]/g, "");
  }

  function obterObraKeyResumo(row) {
    const detalhes = safeJsonParse(row && row[COLS.DETALHES_JSON], {});
    const metaKey = String((detalhes && detalhes.meta_obra_key) || "").trim();
    return metaKey || normalizarObraKeyResumo(row && row[COLS.OBRA]);
  }

  function contarObrasReaisResumo(dados) {
    const chaves = new Set();

    (Array.isArray(dados) ? dados : []).forEach(item => {
      const row = Array.isArray(item && item.content) ? item.content : [];
      const chave = obterObraKeyResumo(row);
  ...
  function renderizar(dadosOriginais) {
    ...
    const totalObrasResumo = currentStatusFilter === 'FIRMADAS' ? contarObrasReaisResumo(dados) : dados.length;
    const divisorCustoMedio = totalObrasResumo > 0 ? totalObrasResumo : dados.length;
    const custoMedio = divisorCustoMedio > 0 ? (totVal / divisorCustoMedio) : 0;
    document.getElementById('resumoObras').innerText = totalObrasResumo;
