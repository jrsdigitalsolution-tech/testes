const ITENS = ["BBA/ELET.", "MT", "FLUT.", "M FV.", "AD. FLEX", "AD. RIG.", "FIXADORES", "SIST. ELÉT.", "PEÇAS REP.", "SERV.", "MONT.", "FATUR."];
  ...
  function callServer(method, args, onSuccess, onError) {
    ...
    const timer = setTimeout(() => { finalizeError(`Tempo excedido.`); }, timeoutMs);
  ...
  function formatMoneyBR(value) {
    return parseMoneyFlexible(value).toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
  }
  ...
  const mapaOrdenacaoCabecalho = {
    "OBRA": "obra", "CLIENTE": "cliente", "VALOR": "valor", "ITEM": "itemGeral", "CATEGORIA": "categoriaGeral", "CATEG. / SEGMENTO": "categoriaGeral",
  ...
  function ordenarDados(dados) {
    ...
    return dados.slice().sort((a, b) => {
      ...
      // Lógica simplificada de ordenação para manter performance
      if (vA === vB) return 0;
  ...
  function lidarCliqueLinha(idx) {
    const status = String(dadosLocais[idx].content[COLS.STATUS_PROPOSTA] || '').trim();
  ...
  function renderizar(dadosOriginais) {
    ...
    if (!isGeralView) {
      ...
      dadosOrdenados.forEach(dO => {
        ...
        for (let j = COLS.ITEM_INICIO; j <= COLS.ITEM_FIM; j++) {
          ...
          html += `<td><span class="${cl}">${isStatusDate(c) ? formatDateDisplayBR(c) : c}</span></td>`;
          if(c !== "N/A" && c !== "") {
              let mbClass = "mc-chip ";
              if (c === "OK") mbClass += "mc-ok"; else if (c === "?") mbClass += "mc-qm"; else if (isStatusDate(c)) mbClass += "mc-dt";
              miniBadgesMobile += `<div class="${mbClass}"><span class="mc-chip-lbl">${nomeItem}</span><span class="mc-chip-val">${isStatusDate(c) ? formatDateDisplayBR(c) : c}</span></div>`;
          }
        }
      ...
      dadosOrdenados.forEach(dO => {
        ...
        // CARTÃO MOBILE - GERAL (Com as 3 primeiras palavras do Item)
    ...
    if (dados.length === 0) {
      ...
      if(mobileContainer) mobileContainer.innerHTML = `<div class="text-center py-5 text-muted"><i class="bi bi-folder2-open d-block mb-2" style="font-size: 3rem; opacity: 0.5;"></i><p>Nenhuma obra.</p></div>`;
    } else {
  ...
  function carregar() {
    ...
    }, msg => { document.getElementById('tabBody').innerHTML = `<tr><td colspan="20" class="text-center py-5 text-danger"><h5 class="fw-bold">Falha ao Ler Banco</h5></td></tr>`; });
  }
